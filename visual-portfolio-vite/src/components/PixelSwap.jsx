...
function PixelSwap({
  ...
  const reverseCurtain = !!(curtain && transition && !transition.to);
  ...
  const config = { duration, pixelDuration, pixelSpin, pixelScale, pixelRadius, fade, easing, onComplete, curtain };
  ...
  useEffect(() => {
    ...
    const isReverseCurtain = settings.curtain && !to;
    ...
    // A curtain uncover must keep cloning the opaque second layer while its
    // full-size base is hidden. Reversing the pixel animation then reveals the
    // page below instead of trying to paint transparency over an opaque layer.
    const source = layerRefs.current[isReverseCurtain ? 1 : to ? 1 : 0];
