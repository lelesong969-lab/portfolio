...
test("all five projects provide complete English detail copy", async () => {
  const data = await read("src/data/portfolio.ts");

  assert.equal((data.match(/^\s{4}detailEn:\s*\{/gm) ?? []).length, 5);
  assert.equal((data.match(/^\s{6}galleryCopy:\s*\[/gm) ?? []).length, 5);
  assert.equal((data.match(/^\s{6}externalBusinessMeanings:\s*\[/gm) ?? []).length, 5);
});
...
test("project detail pages render from the active language", async () => {
  ...
  assert.match(detail, /Explore the complete charts, process documentation, and design evidence/);
  assert.match(detail, /NEXT PROJECT/);
});
